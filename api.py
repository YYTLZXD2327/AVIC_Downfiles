...
def delete_file():
    ...
    response = requests.post(f'{url}/{endpoint}', auth=('', password))
...
def download_file():
    ...
    if not url.startswith('http://') and not url.startswith('https://'):
        url = protocol + url
    file_to_download = file_to_download_entry.get()
    endpoint = f'download/{file_to_download}'
    response = requests.post(f'{url}/{endpoint}')
    
    if response.status_code == 200:
        with open(file_to_download, 'wb') as f:
            f.write(response.content)
        result_text.insert(tk.END, "文件下载成功\n")
    else:
        result_text.insert(tk.END, f"下载文件失败，状态码: {response.status_code}\n")
...
def upload_file():
    ...
    response = requests.post(f'{url}/upload', files=files, auth=('', password))
...
def get_file_list(file_list_text):
    ...
    if not url.startswith('http://') and not url.startswith('https://'):
        url = protocol + url
    response = requests.post(f'{url}/api/files')
...
# 添加文件名输入框
file_to_download_label = tk.Label(root, text="要下载的文件名:", bg='#f0f0f0', font=("Arial", 12))
file_to_download_label.pack()

file_to_download_entry = tk.Entry(root, width=60, font=("Arial", 12))
file_to_download_entry.pack(pady=10)
...
download_button = tk.Button(root, text="下载文件", command=download_file, bg='#33FF57', fg='white', font=("Arial", 12))
